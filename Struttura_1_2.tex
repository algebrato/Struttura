...
\newl{T(\vet{q}) &=& \abs{\sum_{n_1,n_2,n_3} e^{-i\vet{q}\cdot\vet{R}_n} } ^2 = \sum_{\vet{R}_m = \vet{R}_n} e^{-i\vet{q}\cdot(R_n-R_m)} + \sum_{\vet{R}_n\neq \vet{R}_m} e^{-i\vet{q}\cdot(R_n-R_m)} = \nonumber\\
	&=& N + \sum_{R_n}\left(\sum_{R_n\neq R_m} e^{-i\vet{q}\cdot(R_n-R_m)}\right). 
}
Estendo al continuo
\newl{
	T(\vet{q}) &=& N + N\int d^3r\, e^{-i\vet{q}\cdot(R_n-R_m)}P(R_n-R_m) = \nonumber\\
