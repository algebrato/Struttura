...
dove $f({\vet k})$ indica la densità di portatori di carica sulla banda. \`E abbastanza intuitivo capire che se $f({\vet k})=1$, cioè se la banda è piena, allora $\vet J = 0$ in quanto il profilo velle velocità sarebbe completo per ogni $\vet k$, integrando tutto nella prima zona di Brillouin, si integra $\vet{v_k}$ che è periodica dispari. Questo fa annullare l'integrale (\ref{dens:corr}). Se la banda è semipiena allora ed è presente una certa forza esterna allora in questo caso si avrebbe ${\vet J} \neq 0$. La forza esterna è data applicata inserendo il materiale in un campo elettrico, da questo fatto ecco giustificata la legge di Ohm
...
\newl{{\vet J} = -e \int \frac{d^3k}{(2\pi)^3} v_k \text{...c'è qualcosa che non mi torna}}
